...
const signup = async function(req, res) {
  ...
  const signin = async(req,res,next)=> {
    ...
    if(!match){
      ...
      const syncUserDetails = await SyncUser.findOne({email : email.toLowerCase()}).select('-tracklist').select('-password').exec()
...
const getsyncuserinfo = async (req,res,next)=>{
  ...
  const details = await SyncUser.findOne({_id : userId}).populate('tracklist', "artWork trackTitle mainArtist trackLink duration genre mood writers").select('-password').exec()
